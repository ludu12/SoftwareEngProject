...
public class NotificationCenter : MonoBehaviour
{
	...
	//Do not spawn notification centers if the application is quitting. Added by Amos as per http://forum.unity3d.com/threads/97598-trying-to-use-Notification-Center-from-the-wiki
...
//	    }
	...
	public void PostNotification (Notification aNotification) {
	    ...
	    if (notifyList == null) { 
			//Debug.Log(aNotification.name + " from " + aNotification.sender);
		...
		/*** special ALL category will notify subscribers whenever any notification is sent ***/
		//notify listeners to ALL
		notifyList = (List<Component>)notifications["ALL"];
		if (notifyList == null) {
			return; 
		}
		...
		// Create an array to keep track of invalid observers that we need to remove
		observersToRemove = new List<Component>(); //change from original
		...
		foreach (Component observer in notifyList) {
			...
			if (!observer) { observersToRemove.Add(observer);
				...
				// If the observer is valid, then send it the notification.  The message that's sent is the name of the notification.
				observer.SendMessage("NotificationSent", aNotification, SendMessageOptions.DontRequireReceiver);
			}
		}
...
public class NotificationHashTable {
	...
	//Handle some common value types
	public int frame {
		set 
		{
			if(myHash.ContainsKey("frame")) myHash["frame"] = value;
			else myHash.Add("frame", value);
		} 
		get 
		{
			if(myHash.ContainsKey("frame")) return (int)myHash["frame"];
			else return -403;
		}
	}
	...
	public GameObject go {
		set 
		{
			if(myHash.ContainsKey("gameobject")) myHash["gameobject"] = value;
			else myHash.Add("gameobject", value);
		} 
		get 
		{
			if(myHash.ContainsKey("gameobject")) return (GameObject)myHash["gameobject"];
			else return null;
		}
	}
	...
	public Vector3 position {
		set 
		{
			if(myHash.ContainsKey("position")) myHash["position"] = value;
			else myHash.Add("position", value);
		} 
		get 
		{
			if(myHash.ContainsKey("position")) return (Vector3)myHash["position"];
			else return Vector3.zero;	//DANGER
		}	
	}
	...
	public Quaternion rotation {
		set 
		{
			if(myHash.ContainsKey("rotation")) myHash["rotation"] = value;
			else myHash.Add("rotation", value);
		} 
		get 
		{
			if(myHash.ContainsKey("rotation")) return (Quaternion)myHash["position"];
			else return Quaternion.identity;	//DANGER
		}	
	}
	...
	public Color color {
		set 
		{
			if(myHash.ContainsKey("color")) myHash["color"] = value;
			else myHash.Add("color", value);
		} 
		get 
		{
			if(myHash.ContainsKey("color")) return (Color)myHash["color"];
			else return Color.white;	//DANGER
		}	
	}
	...
	public float alpha {
		set 
		{
			if(myHash.ContainsKey("alpha")) myHash["alpha"] = value;
			else myHash.Add("alpha", value);
		} 
		get 
		{
			if(myHash.ContainsKey("alpha")) return (float)myHash["alpha"];
			else return -1;
		}	
	}
	...
	public string label {
		set 
		{
			if(myHash.ContainsKey("label")) myHash["label"] = value;
			else myHash.Add("label", value);
		} 
		get 
		{
			if(myHash.ContainsKey("label")) return (string)myHash["label"];
			else return "";
		}	
	}
